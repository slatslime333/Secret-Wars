...
import { smashHitsTarget } from '../../heroes/abilities/death/smashHit';
...
if (!smashAhead || smashSide || smashBehind) {
  failed += 1;
  console.log(`FAIL  smash capsule  ahead=${smashAhead} side=${smashSide} behind=${smashBehind}`);
} else {
  console.log('ok  smash capsule  forward hit, side/behind miss');
}
